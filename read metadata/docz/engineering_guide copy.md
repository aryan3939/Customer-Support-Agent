...
5. [0.4 Current Runtime Alignment Update (2026-04-12)](#04-current-runtime-alignment-update-2026-04-12)
6. [1. Problem Framing And Origin Story](#1-problem-framing-and-origin-story)
7. [2. Product Scope, Non-Goals, And Boundaries](#2-product-scope-non-goals-and-boundaries)
8. [3. Why This Architecture](#3-why-this-architecture)
...
### 0.4 Current Runtime Alignment Update (2026-04-12)

This section exists to make the handbook explicitly consistent with the current codebase as of April 12, 2026. The guide remains intentionally long-form, and some older interview or ADR material deeper in the document may still refer to legacy terminology. When there is a conflict, the current runtime behavior described here and in the refreshed core sections below should win.

Current-state alignment summary:

1. The active graph entry point is `classify_intent`, not `route_query`.
2. The graph now routes into four primary outcomes:
   - `retrieve` for `docs_qa`
   - `generate_generic` for `generic_meta`
   - `generate_out_of_scope` for unrelated prompts
   - `clarify_question` for ambiguous or low-confidence queries
3. `route_query` and `generate_direct` still exist in `src/agent/nodes.py`, but only as backward-compatible shims and aliases rather than the primary graph path.
4. The active retrieval path in `src/agent/nodes.py` does perform hybrid retrieval:
   - vector similarity search
   - surrounding-chunk expansion
   - keyword search
   - reciprocal rank fusion
5. `src/agent/tools.py` remains useful as a future seam for tool-calling, but the live graph does not call those tools today.
6. Current LLM defaults in `src/config.py` are:
   - provider `openai`
   - model `gpt-4o`
   - base URL `https://api.openai.com/v1`
7. Current chunking defaults are `1500` characters with `300` overlap, not the older `2000` and `400` values referenced in legacy sections.
8. The FastAPI layer currently exposes:
   - `GET /api/health`
   - `GET /api/stats`
   - `POST /api/chat`
   - `POST /api/chat/reset`
9. Streaming API behavior was not found in inspected runtime artifacts.
10. The evaluation scripts remain useful, but they are not perfect mirrors of the live graph:

- `run_eval.py` evaluates vector retrieval only
- `run_ragas.py` skips grading and rewrite and therefore measures a simplified path

Practical reading rule:

1. Treat sections 4, 6, 15, 17, 19, 21, 22, 32, 39, 38, and 61 as the refreshed implementation-aligned spine of the guide.
2. Treat older terminology in interview banks and legacy ADR language as historical context unless a section is explicitly refreshed below.
...
flowchart LR
   ...
   H[User Query] --> I[Classify Intent]
   I -->|generic_meta| J[Generate Generic]
   I -->|out_of_scope| K[Generate OOS Redirect]
   I -->|ambiguous| L[Clarify Question]
   I -->|docs_qa| M[Hybrid Retrieve]
   M --> N[Grade Documents + Evidence Score]
   N -->|Weak evidence + retries left| O[Rewrite Query]
   O --> M
   N -->|Enough evidence or retries exhausted| P[Generate Grounded Answer]
   J --> Q[Response]
   K --> Q
   L --> Q
   P --> Q
...
```mermaid
flowchart TD
   A[Incoming User Message] --> B[classify_intent]
   B --> C{route_by_intent}
   C -->|generic_meta| D[generate_generic]
   C -->|out_of_scope| E[generate_out_of_scope]
   C -->|ambiguous| F[clarify_question]
   C -->|docs_qa| G[retrieve]
   G --> H[grade_documents]
   H --> I{should_rewrite}
   I -->|rewrite| J[rewrite_query]
   J --> G
   I -->|generate| K[generate]
   D --> L[Return AI Message]
   E --> L
   F --> L
   K --> L
```
...
1. `classify_intent` first applies rule-based shortcuts, then falls back to LLM classification when needed.
2. `route_by_intent` sends the query to one of four branches: `generate_generic`, `generate_out_of_scope`, `clarify_question`, or `retrieve`.
3. If the branch is `retrieve`, the graph runs hybrid retrieval: vector search, context expansion, keyword search, and reciprocal rank fusion.
4. `grade_documents` checks each retrieved chunk for relevance and computes `evidence_score`.
5. `should_rewrite` checks whether the evidence is weak and whether rewrite budget remains.
6. If evidence is weak and retries remain, `rewrite_query` runs and loops back to `retrieve`.
7. If evidence is strong enough, or retries are exhausted, `generate` composes context and creates the final answer.
8. Output message is appended to conversation history.
...
[route_by_intent]
   ...
   v              v                  v                  v
[generic_response] [oos_redirect] [clarify]        [retrieve]
                                                    |
                                                    v
                                            [grade_documents]
                                                    |
...
2. Moderate max tokens to balance grounding, latency, and answer completeness.
3. The `ChatOpenAI` wrapper is still used because the code is written against an OpenAI-compatible interface, even though the default provider is now the native OpenAI endpoint.
...
1. The live graph path in `src/agent/nodes.py` does perform context expansion through `_expand_to_parent(...)` after vector retrieval.
2. `src/agent/tools.py` also contains expansion logic for tool-oriented retrieval helpers.
3. This means expansion exists in both places today, but the duplication is architectural debt rather than missing functionality.
...
2. Overlap: `300` characters.

Rationale:

1. Small enough to improve retrieval precision on technical sections.
2. Large enough to preserve semantic completeness within documentation subsections.
3. Overlap reduces boundary fragmentation without making neighboring chunks too redundant.
...
1. `Secrets.llm_api_key()` chooses which provider key to return based on `LLMConfig.PROVIDER`.
2. The wrapper is now multi-provider by design, even though the current default is OpenAI.
...
### 10.1 Actual Chunking Architecture

The current chunker is explicitly **two-stage**, not a single recursive splitter with markdown-ish separators.
...
2. splits on:
   ...
   - `###` -> `h3`
   - `####` -> `h4`
3. preserves headers in the text with `strip_headers=False`

Stage 2:

1. `RecursiveCharacterTextSplitter`
2. runs only for sections that still exceed `ChunkingConfig.CHUNK_SIZE`
3. uses the configured overlap and separator order for fallback splitting

This means the chunker is fundamentally **header-first, recursive-second**.

### 10.2 Stage 1: Header-Based Structural Split

The first pass converts the document into section-level units before any size-based splitting happens.

Why this matters:

1. Chunks do not cross major section boundaries unless absolutely necessary.
2. Each resulting section carries heading metadata from the markdown hierarchy.
3. Small sections stay intact instead of being split just because a generic splitter ran first.

### 10.3 Stage 2: Recursive Fallback For Oversized Sections

Only oversized sections go through `RecursiveCharacterTextSplitter`.

Actual separator order in code:
...
Current defaults from `ChunkingConfig`:

1. chunk size: `1500`
2. overlap: `300`

This preserves the "small-to-medium coherent section" preference while still safely splitting long procedural or reference-heavy sections.

### 10.4 Heading Breadcrumb Prefix For Sub-Chunks

For oversized sections that need recursive splitting, the code builds a heading breadcrumb with `_build_heading_prefix(headers)`.

Example behavior:

1. metadata like `{"h2": "Connectors", "h3": "Snowflake"}`
2. becomes a prefix like `"Connectors > Snowflake\n\n"`
3. that prefix is prepended only when the sub-chunk does not already begin with a markdown heading

This is an embedding-quality optimization. It keeps section context attached to sub-chunks that would otherwise lose the semantic meaning of their parent heading hierarchy.

### 10.5 Chunk Cleaning After Splitting

Every chunk, including recursively produced sub-chunks, is passed through `_clean_chunk_content(...)`.

Observed cleanup behavior:

1. remove Mintlify empty-link artifacts again via `strip_empty_links`
2. remove lone-number lines
3. collapse runs of blank lines
4. trim leading and trailing whitespace

This second-pass cleaning is important because split boundaries can expose formatting artifacts that were harmless at page level but noisy at chunk level.

### 10.6 Section Heading Extraction

The guide should be precise here because the code does more than just "first heading wins."

`_extract_section_heading(...)` uses this priority:

1. `h3` metadata
2. `h4` metadata
3. `h2` metadata
4. first heading found inside content
5. empty string fallback

The function also re-cleans the heading text with `strip_empty_links(...)`.

So the section heading is a best-effort semantic label, not merely the first line of the chunk.

### 10.7 Chunk Acceptance Rules

The chunker does not keep every split fragment.

Observed filtering behavior:

1. cleaned section or sub-chunk must be non-empty
2. cleaned content must be longer than 20 characters

This is a low-cost noise filter that prevents tiny fragments from entering the embedding and retrieval pipeline.
...
6. heading lineage fields such as `h2`, `h3`, and `h4` when present

Important nuance:

1. `chunk_index` is assigned during chunk creation.
2. `total_chunks` is only added after the full page has been chunked, in a second pass across the produced chunk list.
3. Metadata is page-aware and section-aware, which is useful for retrieval explainability and future metadata filtering.

### 10.9 Why This Design Fits This Codebase

This chunker is tailored for technical documentation rather than arbitrary prose.

Benefits grounded in the implementation:

1. header-first splitting preserves documentation structure better than a plain recursive splitter
2. section metadata gives retrieval and debugging much better observability
3. recursive fallback handles oversized sections without throwing away structural context
4. breadcrumb prefixing improves sub-chunk embeddings for large sections
5. strict post-cleaning reduces noise before vectorization
...
### 11.2 Actual Model Construction

The model is instantiated as:

1. `SentenceTransformer(model_name, device=device)`
2. with `model_name = EmbeddingConfig.MODEL`
3. and `device = EmbeddingConfig.DEVICE`
...
There is no custom pooling, normalization, prompt-prefixing, or provider API wrapper here. It is a direct local sentence-transformers model.

### 11.3 Initialization Checks

On initialization:

1. the model is loaded once
2. a test embedding is generated from the string `"test"`
3. the actual embedding dimensionality is compared against `EmbeddingConfig.DIMENSIONS`
4. a mismatch raises a clear `ValueError`

This prevents silent schema incompatibility with VECTOR dimension constraints.

This check is especially important because the PostgreSQL schema pins `documents.embedding` to `VECTOR(384)`.
...
1. `embed_text(...)` returns `list[float]`
2. `embed_batch(...)` returns `list[list[float]]`

This matters because downstream storage code expects Python lists, not numpy arrays.

### 11.5 Batch Embedding Behavior

`embed_batch(...)` is more specific than the older guide implied.

Observed behavior:

1. returns early with `[]` when `texts` is empty
2. defaults to `batch_size=64`
3. calls `SentenceTransformer.encode(...)` with `convert_to_numpy=True`
4. shows a progress bar only when more than 100 texts are embedded

This is a practical ingestion-oriented interface rather than a generic abstraction over many embedding backends.

### 11.6 Practical Trade-Off

CPU embeddings are slower than GPU but maximize portability and reduce setup complexity in review environments.

Additional implications from the code:

1. local embeddings avoid recurring API cost during ingestion
2. query-time embedding latency stays bounded and local
3. changing to a different embedding model with a different dimension would require both config and schema updates
4. because this is a singleton, the first call pays the model-load cost and later calls reuse it
...
1. Return ranked chunks with source provenance across dense, sparse, and context-expansion paths.

Execution:

1. Query embedding is passed to `match_documents` for dense retrieval.
2. Raw query text is passed to `keyword_search` for sparse retrieval.
3. `match_documents` returns chunk content plus `source_url`, `source_title`, and `chunk_index`.
4. Application code may call `get_surrounding_chunks` for top vector hits to reconstruct a larger parent context.
5. Dense and sparse lists are fused in Python with reciprocal rank fusion.

Plain-text flow:

```text
[query embedding] -> [match_documents RPC] -> [dense rows]
[query text]      -> [keyword_search RPC]  -> [sparse rows]
[top dense rows]  -> [get_surrounding_chunks RPC] -> [expanded dense rows]
[dense + sparse rows] -> [RRF fusion in Python] -> [ranked retrieval set]
```
...
### 13.1 Retrieval Stack Overview

The current retrieval subsystem is intentionally hybrid and split across database-native ranking plus application-side orchestration.

Dense retrieval components:

1. local query embedding via `EmbeddingService`
2. `match_documents(...)` RPC over `pgvector`
3. HNSW index on `documents.embedding`

Sparse retrieval components:

1. raw query text
2. `keyword_search(...)` RPC over PostgreSQL full-text search
3. GIN index on `to_tsvector('english', content)`

Context-enrichment component:

1. `get_surrounding_chunks(...)`
2. used to reconstruct larger parent context around strong vector hits

Fusion layer:

1. reciprocal rank fusion (RRF) in `src/agent/nodes.py`
2. deduplication primarily by `source_url`

Why this architecture exists:

1. dense retrieval handles paraphrase and semantic similarity
2. sparse retrieval handles exact product names, connector names, and literal terms
3. surrounding-chunk expansion gives generation more context than a single hit-sized chunk
4. RRF avoids trying to compare incompatible raw score scales from cosine similarity and `ts_rank`

#### 13.1.1 What The System Actually Retrieves

The atomic retrieval unit in the database is a chunk row in `documents`, not a whole page.

That matters because the retrieval path deliberately uses two different granularities:

1. search granularity is chunk-sized, which improves precision
2. fusion and answer grounding often drift toward page-local context because `_expand_to_parent(...)` and `_doc_key(...)` collapse multiple hits around the same `source_url`

In practice, the runtime behaves like:

1. find the best local chunk
2. reconstruct nearby page context
3. keep one strong candidate per page where possible

This is a practical compromise between sentence-level precision and page-level answerability.

#### 13.1.2 Why Hybrid Retrieval Is The Right Default For Docs

Documentation corpora are not purely semantic and not purely lexical.

They contain:

1. conceptual explanations such as deployment patterns or lineage behavior
2. literal identifiers such as connector names, env vars, and API paths
3. structured sections where neighboring paragraphs are often required to answer correctly

Using only one retrieval mode would underperform on at least one of those shapes:

1. vector-only retrieval misses some literal-term lookups
2. keyword-only retrieval misses paraphrases and conceptual phrasing
3. chunk-only retrieval without expansion can return overly narrow evidence

### 13.2 Dense Retrieval: `match_documents`

Inputs:

1. `query_embedding VECTOR(384)`
2. `match_threshold FLOAT DEFAULT 0.5`
3. `match_count INT DEFAULT 5`
4. `filter_category TEXT DEFAULT NULL`

Behavior:

1. joins `documents d` with `sources s`
2. keeps only `s.is_active = TRUE`
3. computes similarity as:

```text
similarity = 1 - (d.embedding <=> query_embedding)
```

4. applies the threshold:

```text
similarity > match_threshold
```

5. optionally restricts by `section_category`
6. orders by vector distance ascending:
...
#### 13.2.1 The Actual Similarity Math

The SQL computes:

```text
cosine_distance = d.embedding <=> query_embedding
cosine_similarity = 1 - cosine_distance
```

This is important because the application threshold of `0.5` is therefore a cosine-similarity threshold, not an arbitrary opaque score.

Interpretation:

1. a similarity close to `1.0` means strong semantic alignment
2. a similarity just above `0.5` is plausible but not especially strong
3. lower scores are filtered out before they ever reach the Python graph

#### 13.2.2 What The Threshold Really Does

The threshold is the first precision-recall gate in the live retrieval path.
...
1. if a chunk scores `0.49`, it is excluded entirely from dense retrieval output
2. if a chunk scores `0.51`, it survives and can still be expanded, fused, graded, and generated from
3. if all dense candidates score below `0.5`, the dense side contributes nothing and the sparse side must carry recall

So the threshold is not a final confidence score. It is an admission criterion for which candidates get to participate in later stages.

Why this is useful:

1. it prevents obviously weak semantic matches from polluting fusion
2. it reduces downstream grading cost
3. it makes low-evidence cases more likely to trigger the rewrite loop instead of speculative generation

Why it is risky if tuned badly:

1. too high means missed paraphrases and more empty dense result sets
2. too low means more noise reaches grading and generation

#### 13.2.3 Why HNSW + Cosine Similarity
...
1. `all-MiniLM-L6-v2` is a semantic embedding model, so cosine similarity is a natural metric
2. HNSW gives efficient approximate nearest-neighbor retrieval for moderate corpus sizes
3. retrieval stays inside PostgreSQL instead of requiring a separate vector store service

Parameter intuition:

1. `m = 16` controls graph connectivity; higher values generally improve recall and memory cost
2. `ef_construction = 64` controls how much work is spent while building the HNSW graph; higher values generally improve index quality and slow build time

Trade-off:

1. approximate search improves speed, but exact ranking guarantees are relaxed
2. maintainers need to understand vector index semantics, not just plain SQL filtering

#### 13.2.4 Why The Optional Category Filter Matters

`match_documents(...)` supports `filter_category`, but the current live graph does not pass category filters at query time.

Why this still matters:

1. the retrieval layer is already prepared for category-constrained search
2. future tuning could filter to connectors, deployment, or concepts when intent becomes more specific
3. the capability exists in the DB today even if the app does not yet exploit it

#### 13.2.5 Dense Retrieval Benefits And Limits

Dense retrieval is best understood as semantic recall infrastructure.

Strengths:

1. good for paraphrases
2. good for questions that do not literally quote the docs
3. good for concept matching across wording variation

Limits:

1. weaker for exact opaque names
2. weaker when short identifiers dominate meaning
3. can return topically related but answer-irrelevant chunks unless later filtered

### 13.3 Sparse Retrieval: `keyword_search`

Inputs:

1. `search_query TEXT`
2. `match_count INT DEFAULT 5`

Behavior:

1. builds a lexical search condition with:
...
3. filters to active sources
4. ranks results with:
...
#### 13.3.1 Why PostgreSQL Full-Text Search Was Used

This choice is especially useful for documentation retrieval because many critical queries are not "semantic concept only" queries. They are exact or near-exact term lookups such as:

1. connector names
2. configuration keys
3. acronyms
4. API names
5. product-specific phrases

Dense retrieval is good at "what concept is this about?" but weaker at "find the exact literal page that mentions this term."

#### 13.3.2 What The `english` Text Search Configuration Implies

Both `to_tsvector` and `websearch_to_tsquery` use the PostgreSQL `english` configuration.

That usually means:

1. stemming is applied, so related word forms can match each other better
2. common stop words may be discarded
3. retrieval remains lexical, not semantic

This is helpful for natural documentation prose, but it also means the sparse layer is not a raw substring matcher. It is PostgreSQL full-text search with language-aware tokenization.

#### 13.3.3 Why `websearch_to_tsquery` Instead Of A Simpler Parser

`websearch_to_tsquery` is a pragmatic choice because it supports more natural user phrasing and search operators than a rigid term parser.

Benefits:

1. supports quoted phrases
2. supports AND/OR/NOT style behavior
3. better matches human search expectations for documentation lookup

Trade-off:

1. ranking is lexical, not semantic
2. if terminology differs from the literal user phrasing, sparse retrieval alone can still miss

#### 13.3.4 Sparse Retrieval Benefits And Limits

Strengths:

1. strong on literal identifiers
2. strong on exact names and quoted phrases
3. helpful when embeddings blur together similar product concepts

Limits:

1. does not understand paraphrase the way embeddings do
2. ranking is tied to token overlap and FTS weighting, not deep semantic equivalence
3. if the docs use a different exact term than the user, recall can collapse

### 13.4 Context Expansion: `get_surrounding_chunks`
...
1. selects chunks from the same source where:

```text
chunk_index BETWEEN target_chunk_index - context_window
               AND target_chunk_index + context_window
```

2. preserves chunk order with `ORDER BY d.chunk_index`
3. returns `id`, `content`, `chunk_index`, and `metadata`

#### 13.4.1 Why This RPC Exists

The dense match often finds the most semantically relevant small chunk, but generation usually benefits from slightly larger local context.

This creates a "small-to-big" pattern:

1. search on smaller chunks for precision
2. expand into neighboring chunks for answer-generation richness

#### 13.4.2 Current Expansion Window

From `RetrievalConfig`:

1. `EXPAND_CONTEXT = True`
2. `CONTEXT_WINDOW = 1`

So a typical expansion fetches:

1. the matched chunk
2. one chunk before it
3. one chunk after it

when those neighbors exist.

#### 13.4.3 Approximate Context Size Calculation
...
the theoretical maximum unique text span around one center hit is approximately:

```text
unique_span ~= CHUNK_SIZE + 2 * (CHUNK_SIZE - CHUNK_OVERLAP)
             ~= 1500 + 2 * 1200
             ~= 3900 characters
```

The raw concatenated context can be closer to:

```text
3 * 1500 = 4500 characters
```

plus separators, because overlap text is repeated when neighboring chunks are stitched together verbatim.

Important nuance:

1. this is an upper-bound intuition, not a guaranteed size
2. actual chunks are often shorter because header-based splitting can produce smaller sections
3. edge chunks near the start or end of a page will have fewer neighbors

#### 13.4.4 Why Only The Dense Side Is Expanded

The live graph expands vector hits before fusion but does not expand keyword hits.

Why that makes sense in the current implementation:

1. vector retrieval is the main semantic recall path and benefits most from richer local context
2. lexical hits already serve as exact-term confirmation signals in fusion
3. expanding both sides would add more DB calls and larger prompts

Trade-off:

1. keyword-only wins can remain narrower than expanded vector wins
2. this slightly biases the system toward richer dense candidates during downstream grading and generation

### 13.5 Application-Side Fusion Theory

The DB does not perform hybrid fusion. That happens in Python in `_merge_hybrid_results(...)`.

Why this is sensible:

1. dense and sparse result sets have different raw score scales
2. `similarity` and `ts_rank` are not directly comparable
3. rank-based fusion is easier to reason about and maintain than score calibration

#### 13.5.1 Why The Order Of Operations Matters

The runtime order is:

1. dense retrieval
2. dense context expansion
3. sparse retrieval
4. hybrid fusion
5. LLM grading
...
1. dense hits are enriched before they compete in fusion
2. keyword hits contribute precision signals without forcing the DB layer to understand fusion semantics
3. grading sees already-fused candidates instead of independently judging two disconnected ranked lists

### 13.6 Reciprocal Rank Fusion (RRF)
...
where each `rank_i` is that document's 1-based rank in one retrieval list.

#### 13.6.1 What The Constant Means

With `rrf_k = 60`, the contribution of a single list rank is:
...
1. it keeps the difference between nearby top ranks relatively modest
2. it strongly rewards appearing in multiple lists

#### 13.6.2 Why Multiple Signals Matter

Example:

1. document A appears only in vector rank 1
2. document B appears in vector rank 3 and keyword rank 4

Scores:

```text
A = 1/61 ~= 0.01639
B = 1/63 + 1/64 ~= 0.01587 + 0.01563 = 0.03150
```

So document B wins even though it was not rank 1 anywhere, because two independent retrieval modes agree on it.

That is one of the main reasons RRF is attractive in hybrid retrieval systems.

#### 13.6.3 Why Rank-Based Fusion Was Used Instead Of Score Fusion

Benefits:

1. avoids brittle normalization between cosine similarity and `ts_rank`
2. robust to score-scale mismatch
3. easy to explain and debug

Trade-off:

1. it ignores absolute score magnitudes after ranking
2. a weak rank position in one list still contributes something, even if the underlying raw score was marginal

#### 13.6.4 Worked Fusion Example Across Both Lists

Suppose the ranked lists are:
...
Final fused order is therefore:
...
1. cross-list agreement can outrank a single strong hit
2. a keyword-only document can still survive into the final set if its lexical rank is strong enough

### 13.7 Deduplication Semantics During Fusion

The application uses `_doc_key(doc)` to identify duplicates.

Priority:

1. `source_url`
2. `id`
3. fallback on title + snippet

Practical consequence:

1. when `source_url` is present, fusion deduplicates at page level, not chunk level
2. this intentionally promotes page diversity
3. it also means multiple independently useful chunks from the same page collapse into one fused candidate

This is a design choice, not an accident.

#### 13.7.1 Why Page-Level Dedup Helps And Hurts

Benefits:

1. avoids flooding the final context with near-duplicate chunks from one page
2. increases odds that the answer sees evidence from multiple documentation pages
3. works well with context expansion because one page hit can already carry nearby local context

Costs:

1. two distinct useful sections from the same page cannot both survive as separate fused candidates
2. page-level collapse can hide intra-page diversity when very long pages cover multiple subtopics

This trade-off is especially visible when one documentation page is broad and frequently matched by both dense and sparse paths.

### 13.8 Retrieval Types And Final Ordering

After fusion, each result is labeled as one of:

1. `vector`
2. `keyword`
3. `hybrid`

Final sorting in code is:
...
1. RRF score is the primary decision signal
2. dense similarity breaks ties where available
3. sparse rank helps when lexical candidates need tie-breaking

#### 13.8.1 Cardinality Bounds At Current Defaults

With `TOP_K = 5`:

1. dense retrieval returns at most 5 rows
2. sparse retrieval returns at most 5 rows
3. merged candidates are at most 10 before deduplication
4. final fused output is truncated back to 5

In practice, deduplication often lowers the merged set below 10 because page-level keys collapse overlaps between the two lists.

### 13.9 Why Put Part Of Retrieval In RPCs And Part In Python

Database-side benefits:

1. vector search stays close to indexed data
2. full-text search uses native PostgreSQL ranking
3. surrounding chunk lookup avoids round-tripping raw tables unnecessarily

Application-side benefits:

1. hybrid fusion is easier to express and evolve in Python
2. deduplication and retrieval-type labeling are easier to test at the app layer
3. evidence scoring and rewrite decisions belong naturally in the graph runtime

Trade-off:

1. retrieval behavior spans SQL and Python, so maintainers need literacy in both layers

#### 13.9.1 Query-Time Cost Model At Current Defaults

For one successful docs query without rewriting, the retrieval stack can perform:

1. 1 local query embedding call
2. 1 `match_documents` RPC
3. up to `n` `get_source_by_url(...)` lookups during expansion
4. up to `n` `get_surrounding_chunks(...)` RPCs during expansion
5. 1 `keyword_search` RPC
6. up to `TOP_K = 5` grader LLM calls

where:

```text
n <= number of dense hits <= TOP_K = 5
```

and with `CONTEXT_WINDOW = 1`, each surrounding-chunk RPC can return up to 3 chunk rows.

So the retrieval-side DB work per pass is bounded but not trivial:

1. one vector RPC
2. one keyword RPC
3. up to five source lookups
4. up to five surrounding-chunk RPCs

If evidence remains weak, the retrieve-grade block can repeat. With `REWRITE_MAX_ATTEMPTS = 2`, the system can execute up to 3 total retrieval passes for one hard query:

1. initial query
2. first rewritten query
3. second rewritten query

That makes the retrieval design quality-oriented rather than minimal-latency-oriented.

### 13.10 Retrieval Failure Modes And Their Meaning

1. dense miss, sparse hit:
   likely exact-term query where embeddings were less decisive
2. sparse miss, dense hit:
   likely paraphrased concept question
3. both miss:
   corpus gap, threshold too high, or vocabulary mismatch
4. good retrieval but weak evidence score:
   too few retained docs, low top similarity, or low keyword confirmation
5. many hits from one page:
   chunking and page-level deduplication are interacting strongly

#### 13.10.1 How To Read Failure Signatures In Practice

Useful interpretation patterns:

1. high dense similarity but grader rejection usually means topical similarity without answer relevance
2. repeated keyword-only survivors usually indicate the corpus depends heavily on exact identifiers for that topic
3. repeated rewrites with no sparse hits often indicate missing literal terms or corpus coverage gaps
4. repeated rewrites with some hits but low evidence usually indicate marginal matches rather than total recall failure

### 13.11 Retrieval Sequence Diagram
...
sequenceDiagram
    ...
    U->>E: embed_text(query)
    E-->>VS: query vector
    VS->>DB: match_documents(query_embedding, threshold, top_k)
    DB-->>VS: dense chunk hits
    loop per surviving dense hit
        VS->>DB: get_source_by_url(source_url)
        VS->>DB: get_surrounding_chunks(source_id, chunk_index, window)
        DB-->>VS: neighboring chunks
    end
    U->>TS: search(query, top_k)
    TS->>DB: keyword_search(search_query, top_k)
    DB-->>TS: lexical hits
    VS-->>F: expanded dense hits
    TS-->>F: keyword hits
    F-->>G: fused top-k candidates
    G-->>F: relevance-filtered candidates
```

### 13.12 Why This Retrieval Design Fits The Codebase

This project is a documentation assistant, not a general open-domain search engine. That matters.

The current retrieval design fits because:

1. docs contain both concepts and exact product terms
2. pages are structured enough for chunk-local search plus neighbor expansion
3. corpus size is moderate enough for PostgreSQL-native vector and sparse retrieval
4. the graph can afford a small number of high-quality candidates rather than wide fan-out retrieval
5. hybrid retrieval plus evidence gating is a practical quality/cost balance for this codebase
...
Retrieval-facing methods worth calling out:

1. `similarity_search(...)`
   - defaults `top_k` and threshold from `RetrievalConfig`
   - passes optional `category` through to SQL even though the live graph does not currently use it
2. `get_surrounding_chunks(...)`
   - defaults `window` from `RetrievalConfig.CONTEXT_WINDOW`
   - is the bridge between precise vector hits and expanded parent context
3. `get_source_by_url(...)`
   - is used by `_expand_to_parent(...)` because the vector RPC returns URL/title but not `source_id`
...
1. it is intentionally thin
2. ranking semantics live in SQL, not Python
3. this keeps lexical search behavior easy to inspect in one place

### 14.4 Why Retrieval Adapters Matter

These adapters are thin by design.

Benefits:

1. the database remains the source of truth for dense and sparse ranking
2. Python orchestration can focus on hybrid fusion, not low-level SQL assembly
3. unit boundaries stay clean:
   - SQL for ranking
...
LangGraph allows explicit state transitions, conditional routing, and bounded loops, which are critical for the current implementation. The runtime is no longer just "retrieve or direct answer"; it is an intent-first control system that can:

1. separate documentation queries from meta, out-of-scope, and ambiguous prompts,
2. preserve branch decisions in explicit state,
3. retry retrieval through rewriting when evidence is weak,
4. terminate safely without hidden recursion.
...
5. `intent`: selected route label.
6. `intent_confidence`: classifier confidence score.
7. `route_reason`: short explanation of why the route was selected.
8. `evidence_score`: post-retrieval evidence strength used for rewrite and generation gating.
...
5. `generate`
6. `generate_generic`
7. `generate_out_of_scope`
8. `clarify_question`

Conditional entry:

1. `route_by_intent` chooses among docs retrieval, generic response, out-of-scope redirect, and clarification.
...
1. `generate`
2. `generate_generic`
3. `generate_out_of_scope`
4. `clarify_question`

### 15.4 Intent Classification Behavior

`classify_intent` is the active graph entry point.

Observed logic:

1. Normalize the query text.
2. Apply rule-based intent shortcuts for obvious greetings, help prompts, out-of-scope requests, and too-short messages.
3. If rules do not decide, call the LLM with `INTENT_CLASSIFIER_PROMPT`.
4. Parse JSON output defensively.
5. If the LLM predicts `docs_qa` below the configured confidence threshold, downgrade to `ambiguous`.

Benefits:

1. Saves retrieval cost for non-doc interactions.
2. Lets the assistant ask clarifying questions instead of guessing on vague prompts.
3. Makes the graph safer and more explainable than a direct retrieve-first approach.

### 15.5 Route-By-Intent Behavior

`route_by_intent` translates the selected intent into the actual graph edge:

1. `generic_meta` -> `generate_generic`
2. `out_of_scope` -> `generate_out_of_scope`
3. `ambiguous` -> `clarify_question`
4. `docs_qa` -> `retrieve`

Compatibility note:

1. `route_query` still exists in `src/agent/nodes.py`, but only as a backward-compatible shim for older integrations.
2. The active graph in `src/agent/graph.py` does not use `route_query` as its entry edge anymore.

### 15.6 Retrieve Node Behavior

`retrieve` performs hybrid search with **Reciprocal Rank Fusion (RRF)**:

1. Query embedding using `EmbeddingService`.
2. Vector search using `VectorStore.similarity_search`.
3. Context expansion for vector hits through `_expand_to_parent(...)`.
4. Keyword search using `TextSearch.search`.
5. Merge both result sets in Python using RRF with `rrf_k = 60`.
6. Deduplicate primarily by `source_url` and annotate each result with retrieval type and fused score.

Returns `retrieved_docs` payload.

Important nuance:

1. The current implementation is hybrid in the graph itself, not just in the helper tools.
2. Vector and keyword hits are complementary: semantic recall plus exact-term precision.
3. Surrounding-chunk expansion makes the retrieval path "small-to-big" instead of pure tiny-chunk retrieval.
4. Expansion is applied to vector hits before hybrid fusion, which means semantic winners get richer context before being compared to lexical winners.
5. Keyword retrieval is wrapped in a `try/except`; if lexical search fails, the dense path still proceeds.

Why this ordering makes sense:

1. dense retrieval finds semantically relevant chunks
2. expansion reconstructs more complete local context for those chunks
3. sparse retrieval contributes exact-term evidence
4. fusion then compares whole candidate pages rather than raw chunk fragments where possible

### 15.6.1 Retrieve-Node Calculations That Matter
...
```text
sort by (hybrid_score, similarity, rank) descending
```

This is a deliberately rank-driven retrieval design rather than a "trust one score type everywhere" design.

### 15.7 Grade Node Behavior
...
3. After grading, the node computes `evidence_score` from:
   - top similarity,
   - coverage of retained docs,
...
3. Grading adds LLM latency, but it improves grounding discipline before generation.

### 15.7.1 Why Grade After Retrieval Instead Of Before Generation Only

The guide should be explicit here because this is a core quality control stage.

Benefits:

1. removes semantically nearby but answer-irrelevant chunks before they contaminate generation
2. creates a structured evidence signal for the rewrite loop
3. improves explainability because the system can say it had weak evidence rather than merely "no answer"

### 15.8 Rewrite Loop

If evidence is weak and rewrite attempts are below limit, `rewrite_query` runs and increments counter.

Important correction:

1. The current rewrite decision is not based only on "zero docs survive."
2. The actual gate also considers `evidence_score` and the configured minimum evidence threshold.

This creates bounded self-correction instead of immediate failure.

### 15.8.1 Evidence Score Calculation

The code computes:

```text
top_similarity = max(similarity of retained docs)
coverage = min(number_of_retained_docs / TOP_K, 1.0)
keyword_signal = min(number_of_keyword_or_hybrid_docs / 2, 1.0)
...
```text
rewrite if evidence_score < EVIDENCE_MIN_SCORE
```
...
#### 15.8.2 Example Calculation

Suppose retained docs look like this:

1. top similarity `0.83`
2. 3 retained docs with `TOP_K = 5`
3. 1 doc labeled `hybrid`

Then:

```text
coverage = 3 / 5 = 0.6
keyword_signal = 1 / 2 = 0.5
...
=   0.4565 + 0.21 + 0.05
=   0.7165
->  0.717 after rounding
```

This would generate, not rewrite.

By contrast, one weak dense-only hit might produce:
...
score = 0.55*0.58 + 0.35*0.2 + 0
      = 0.319 + 0.07
      = 0.389
```

That would trigger rewriting because it falls below `0.45`.
...
1. Return a fixed low-evidence fallback message instead of forcing a speculative answer.

Output:

1. Returns a new AI message into state.

### 15.10 Branch-Specific Direct Nodes

The current direct-response nodes are:

1. `generate_generic` for greetings and capability/help prompts.
2. `generate_out_of_scope` for unrelated requests.
3. `clarify_question` for vague or low-confidence documentation requests.

Compatibility note:

1. `generate_direct` still exists as an alias to `generate_generic`.
2. It is retained for older callers, but the graph no longer routes to it directly.

### 15.11 ChatAgent Wrapper
...
1. The runtime memory model is still process-local.
2. The graph is stateful per request, while session continuity is handled outside the graph by `ChatAgent` and `SessionManager`.
...
1. Graph node implementation in `nodes.py` performs retrieval directly and is the active production path.
2. The tool functions are not currently invoked by `build_graph()`.
3. The helper tools are still useful for future function-calling or alternative agent patterns.
4. Because retrieval logic exists in both places, the codebase carries a small duplication risk that maintainers should keep in mind.
...
No streaming endpoint was found in the current runtime artifacts.
...
1. Source URLs are not returned as a structured retrieval artifact from graph state.
2. They are inferred after generation from the response text itself.
3. This is pragmatic for an MVP but weaker than structured citation output.
...
Current required keys by runtime path:

1. `SUPABASE_URL`
2. `SUPABASE_SERVICE_KEY`
3. `SUPABASE_DB_URL`
4. API key for the active LLM provider selected in `LLMConfig`
5. Optional `LANGCHAIN_API_KEY` for LangSmith tracing
...
5. `GET /api/stats` returns non-zero counts after ingestion.
6. `ChatAgent` initialization succeeds with the selected provider and embedding model.
...
1. `run_eval.py` over-fetches vector results, deduplicates by source URL, and then computes HR@K and MRR@K.
2. It does **not** currently exercise keyword retrieval, reciprocal rank fusion, grading, or the rewrite loop.
3. It is therefore a retrieval-health indicator, not a perfect mirror of the live graph path.
...
1. `run_ragas.py` retrieves with vector search only.
2. It intentionally skips the graph grading step to save LLM calls.
3. It does not execute the full intent-route-retrieve-grade-rewrite-generate runtime flow.
4. It remains highly useful, but it is best interpreted as a simplified end-to-end proxy rather than perfect production parity.

### 22.4 Why Both Methods?

Combining offline ranking metrics and AI-as-a-judge provides full visibility. We found that deploying OpenAI GPT-4o vastly improved faithfulness compared to smaller open-source models, while hybrid retrieval directly boosted downstream answer relevancy for edge cases like specific connector names.

### 22.5 Recommended Evaluation Reading Rule

When interpreting eval results:

1. treat `run_eval.py` as retrieval baseline instrumentation,
2. treat `run_ragas.py` as simplified generation-quality instrumentation,
3. do not assume either script fully validates the live LangGraph branch logic,
4. use both metrics plus manual sample runs after any graph-routing or prompt changes.
...
2. Inspect both vector and keyword retrieval behavior rather than assuming one path is missing.
...
3. Verify active provider API key, model name, and base URL availability.
...
"I would persist sessions and add authenticated API access with CORS restriction, then unify the duplicated retrieval logic between `nodes.py` and `tools.py` so the hybrid path stays consistent everywhere."
...
1. The question bank remains intentionally extensive, but some older answers may still mention legacy shorthand such as `route_query`, `generate_direct`, OpenRouter defaults, or "keyword fallback" as future work.
2. Read those items through the runtime-alignment note in section 0.4.
3. The current code-backed interpretation is:
   - active graph entry is `classify_intent`
   - branch routing is via `route_by_intent`
   - graph retrieval is already hybrid
...
3. Retrieval logic is duplicated between the active graph path and helper tools, which increases maintenance risk even though the graph already does hybrid retrieval.
...
1. Aligned the guide with the current intent-first LangGraph runtime.
2. Corrected configuration defaults to the current `openai` + `gpt-4o` settings.
3. Updated workflow diagrams and playbooks to reflect `classify_intent`, `route_by_intent`, and the current branch structure.
4. Corrected setup, API, and evaluation sections to match current code behavior.
5. Added an explicit runtime-alignment note for legacy terminology that may still appear in older deep-dive or interview sections.
...
#### 39.6.1 Two-Stage Split Strategy

The implementation is explicitly two-stage:

1. `MarkdownHeaderTextSplitter` creates section-level units using `##`, `###`, and `####`
2. `RecursiveCharacterTextSplitter` only handles sections that still exceed chunk size

That design is important because it means the chunker is structure-preserving first and size-preserving second.

#### 39.6.2 Header Metadata And Breadcrumb Prefix

For each header-split section:

1. heading metadata is captured in keys like `h2`, `h3`, `h4`
2. `_build_heading_prefix(...)` turns those headers into a breadcrumb string
3. that breadcrumb is prepended to recursive sub-chunks when the sub-chunk does not already begin with a heading

This is an implementation-level quality improvement for embeddings, not just a formatting detail.

#### 39.6.3 Post-Clean Rules

For each section or sub-chunk:
...
#### 39.6.4 Section Heading Extraction

The code does not simply take the first heading in the text.
...
This gives the chunk metadata a more specific section label in many cases.

#### 39.6.5 Metadata Richness

Chunk metadata includes enough provenance to support:

1. citation reconstruction
2. section-aware inspection and debugging
3. future metadata filtering or reranking features
...
4. `chunk_index`
5. `total_chunks`
6. any available `h2`, `h3`, `h4` values
...
1. batch embedding is performed from `texts = [chunk.content for chunk in chunks]`
2. chunk records are rebuilt as dictionaries containing:
   - `content`
   - `embedding`
   - `chunk_index`
   - `metadata`
3. those records are then handed to `VectorStore.store_documents(...)`
...
Thin wrapper around `match_documents(...)` with defaulted `top_k` and threshold.

Important retrieval detail:

1. the wrapper exposes optional category filtering even though the active graph does not use it
2. the returned rows already include source URL/title and similarity, so the app layer does not need a second join for first-pass dense retrieval

#### 39.8.4 Surrounding Chunk Retrieval

`get_surrounding_chunks(...)` wraps the neighborhood-expansion RPC.

Behavior:

1. defaults window from `RetrievalConfig.CONTEXT_WINDOW`
2. returns ordered chunk neighborhoods from the same page
3. enables the small-to-big retrieval pattern in the graph

#### 39.8.5 Utility Methods
...
Builds a `ChatOpenAI` client configured for the active provider in `src/config.py`.

Current behavior:

1. Uses `LLMConfig.MODEL`, `TEMPERATURE`, and `MAX_TOKENS`.
2. Uses `Secrets.llm_api_key()` so the API key source depends on the selected provider.
3. Uses `LLMConfig.BASE_URL`, which currently defaults to `https://api.openai.com/v1`.

This is an OpenAI-compatible abstraction, not an OpenRouter-only one.

#### 39.9.2 `_normalize_query()`

Normalizes text for lightweight routing heuristics.

Behavior:

1. Lowercases.
2. Removes non-alphanumeric noise.
3. Collapses repeated whitespace.

This helps rule-based routing remain stable even with punctuation-heavy user input.

#### 39.9.3 `_rule_based_intent()`

Applies cheap, deterministic intent shortcuts before invoking the LLM classifier.

Observed cases:

1. greetings and assistant-help prompts -> `generic_meta`
2. obvious unrelated prompts like jokes, weather, recipe, movie -> `out_of_scope`
3. very short or empty prompts -> `ambiguous`

This is where the low-cost front-door behavior now lives.

#### 39.9.4 `_parse_intent_response()`

Safely parses the LLM classifier response.

Important detail:

1. It attempts direct JSON parsing first.
2. If that fails, it attempts to recover a JSON object from surrounding text.
3. It clamps confidence to `[0.0, 1.0]`.
4. It defaults to `docs_qa` if parsing is malformed.

This is defensive parsing rather than trusting perfect model output.

#### 39.9.5 `classify_intent()`

This is the active graph entry node.

Behavior:

1. Normalize query.
2. Try rule-based routing first.
3. If rules do not decide, invoke `INTENT_CLASSIFIER_PROMPT`.
4. Parse intent, confidence, and reason.
5. Downgrade low-confidence `docs_qa` to `ambiguous`.

This replaces the older "retrieve-or-direct" entry model with a fuller routing layer.

#### 39.9.6 `route_by_intent()`

Maps the selected intent into the actual graph branch.

Outputs:

1. `generate_generic`
2. `generate_out_of_scope`
3. `clarify_question`
4. `retrieve`

#### 39.9.7 `route_query()`

Retained only as a backward-compatible shim for older integrations.

Important note:

1. It is not the active graph entry edge anymore.
2. It effectively collapses the newer routing model into `retrieve` versus `generate_direct`.
3. The guide should treat it as legacy compatibility behavior.

#### 39.9.8 `retrieve()`

Implements the current hybrid retrieval path.

Behavior:

1. Embed the query using `EmbeddingService`.
2. Run vector similarity search through `VectorStore.similarity_search`.
3. Expand top vector hits into parent context through `_expand_to_parent(...)`.
4. Run keyword search through `TextSearch.search`.
5. Merge both result sets through `_merge_hybrid_results(...)`.

This is the live retrieval path that the graph uses today.

Why this matters:

1. the graph is not "vector-only with optional fallback"
2. hybrid retrieval is first-class runtime behavior
3. expansion happens before fusion, which changes the context shape seen by later stages

#### 39.9.9 `_merge_hybrid_results()`

Combines vector and keyword rankings through reciprocal rank fusion.

Important detail:

1. Result identity is keyed primarily by source URL.
2. The fused result records carry `hybrid_score` and `retrieval_type`.
3. If a page appears in both lists, it becomes a single hybrid candidate.
4. rank contributions use `1 / (60 + rank)` with rank starting from 1.
5. final sorting uses `hybrid_score`, then `similarity`, then `rank`.

This is a rank-fusion design rather than a raw-score calibration design.

#### 39.9.10 `_expand_to_parent()`

Implements small-to-big retrieval.

Behavior:

1. Keep the strongest chunk hit per source page.
2. Fetch surrounding chunks from the same source.
3. Stitch them together into a larger parent block.

This preserves retrieval precision while improving generation context.

#### 39.9.11 `grade_documents()`

Per-doc LLM grading with yes/no verdict plus evidence scoring.

Implementation nuance:

1. Verdict check still uses substring containment of `"yes"`.
2. Grading truncates each document to the first 1000 characters for the grader call.
3. On exception, the document is retained rather than dropped.
4. The node calculates `evidence_score` after filtering.

#### 39.9.12 `_compute_evidence_score()`

Builds the evidence gate used by the rewrite loop.

Current ingredients:

1. top similarity
2. retained-doc coverage relative to `TOP_K`
3. keyword or hybrid retrieval signal

This is one of the key control signals in the agentic runtime.
...
#### 39.9.13 `should_rewrite()`

Decides whether the graph should retry retrieval.

Current logic:

1. only applies to `docs_qa`
2. rewrites when evidence is weak or no docs remain
3. respects `REWRITE_MAX_ATTEMPTS`

So the gate is now evidence-aware, not merely empty-result-aware.

#### 39.9.14 `rewrite_query()`

Asks the LLM to reformulate the query for improved retrieval terms and increments `rewrite_count`.

#### 39.9.15 `generate()`

Two modes:

1. grounded context mode when evidence is above threshold
2. low-evidence fallback message when the threshold is not met

Conversation handling:

1. Uses last six messages for recency control.

#### 39.9.16 `generate_generic()`

Handles greeting, capability, and assistant-help prompts without retrieval.

#### 39.9.17 `generate_out_of_scope()`

Handles unrelated prompts by declining and redirecting toward OpenMetadata help.

#### 39.9.18 `clarify_question()`

Handles ambiguous requests by asking one concise clarifying question with examples.

#### 39.9.19 `generate_direct()`

Retained as a backward-compatible alias to `generate_generic()`.
...
1. entry point -> `classify_intent`
2. `classify_intent` -> conditional branch via `route_by_intent`
3. `retrieve` -> `grade_documents`
4. `grade_documents` -> conditional branch via `should_rewrite`
5. `rewrite_query` -> `retrieve`
6. `generate`, `generate_generic`, `generate_out_of_scope`, and `clarify_question` -> `END`
...
#### 39.11.5 `/api/health` Endpoint

Returns a lightweight runtime health payload including:

1. status
2. active model
3. embedding model
4. LangSmith enabled flag

#### 39.11.6 `/api/chat/reset` Endpoint

Resets in-memory session state for a given `session_id`.
...
1. Embed question.
2. Retrieve vector results with zero threshold and an over-fetch strategy.
3. Deduplicate by source URL.
4. Match expected URL patterns.
5. Compute hit and reciprocal rank.

Important limitation:

1. This walkthrough is retrieval-only.
2. It does not run keyword search, RRF fusion, grading, rewrite, or branch routing.
...
1. `classify_intent` evaluates the input using rules first, then the LLM classifier if needed.
2. `route_by_intent` routes the query to `retrieve` because this example is a documentation question.

### 40.3 Retrieval

1. Query text embedded to 384-d vector.
2. `match_documents` RPC returns top-K chunks with similarity and source metadata.
3. Top vector hits are expanded with surrounding chunks.
4. Keyword search runs in parallel at the logical stage.
5. Both result sets are fused with reciprocal rank fusion.
...
1. If evidence is strong enough -> `generate`.
2. If evidence is weak and rewrite attempts remain -> `rewrite_query` then loop back.
3. If evidence remains weak after retries are exhausted -> `generate` returns the low-evidence fallback path.
...
### ADR-007: Chunk Size And Overlap Tuned For Current Retrieval Quality
...
1. Use 1500 character chunks with 300 overlap.

Consequences:

1. Better retrieval precision on technical subsections.
2. Enough continuity to preserve cross-boundary meaning.
3. Lower redundancy than the older larger-chunk configuration.

### ADR-008: Use OpenAI-Compatible ChatOpenAI Client
...
1. Configure `ChatOpenAI` against the provider selected in `LLMConfig`.
2. Default to the native OpenAI endpoint while keeping the interface portable to other compatible providers.

Consequences:

1. Easy model swap potential remains.
2. Runtime behavior is cleaner because provider selection lives in one config module.
3. Dependency risk shifts from any single gateway to the currently selected provider.
...
3. Network issues to DB or the active LLM provider.
...
A20. Consolidate duplicated retrieval logic between the live graph path and helper tools.
This is ideal because it explicitly balances retrieval quality, ranking quality, and practical trade-offs.
...
5. Provider outage (active LLM provider or DB network path).
...
3. Verify DB and active LLM provider reachability.
...
2026-04-12 (runtime alignment update):

1. Refreshed the core architecture narrative around intent classification and branch-specific generation.
2. Corrected hybrid retrieval, evaluation-parity, and provider-default details.
3. Documented legacy-term compatibility so older interview material can still be read safely.
